...
class OrderTest < ActiveSupport::TestCase
  ...
  test 'should work if the required fields are provided' do
    order = Order.new(first_name: 'foo', last_name: 'bar', address: 'stuff', quantity: '3', credit_card_number: '1', credit_card_code: '1', credit_card_date: "01/#{(DateTime.now.strftime('%y').to_i + 10).to_s}", country: 'USA')
    assert order.valid?
  end
  
  test 'should fail of the credit card date is incorrectly formatted' do
    order = Order.new(first_name: 'foo', last_name: 'bar', address: 'stuff', quantity: '3', credit_card_number: '1', credit_card_code: '1', credit_card_date: 'foo', country: 'USA')
    assert_not order.valid?
  end
  ...
  test 'should fail if Credit Card process returns an error' do
    ...
    CreditCard.stub(:process, raises_exception) do
      order = Order.create(first_name: 'foo', last_name: 'bar', address: 'stuff', quantity: '3', credit_card_date: "01/#{(DateTime.now.strftime('%y').to_i + 10).to_s}", country: 'USA')
